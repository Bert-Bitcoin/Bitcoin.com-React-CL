...
const svgSourceDir = path.resolve(projectRoot, 'src', 'assets-icons');
const outputDir = path.resolve(projectRoot, 'src', 'components', 'asset-icons');
...
async function generateAssetIcons() {
  ...
  if (svgFiles.length === 0) {
    console.warn('[asset-icons] No SVG files found in src/assets-icons.');
    return;
  }
  ...
  for (const file of svgFiles) {
    ...
    // Remove "asset-" prefix and clean the name
    ...
    if (!componentBaseName) {
      componentBaseName = 'Asset';
    }
    if (/^[0-9]/.test(componentBaseName)) {
      componentBaseName = `Asset${componentBaseName}`;
    }
    
    let componentName = `${componentBaseName}AssetIcon`;
    ...
    if (usedNames.has(componentName)) {
      ...
      componentName = `${componentBaseName}${count}AssetIcon`;
    }
  ...
  console.info(`[asset-icons] Generated ${svgFiles.length} asset icon components.`);
}

await generateAssetIcons();
